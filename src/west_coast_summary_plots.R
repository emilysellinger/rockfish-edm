...
# plot results
pdf(here("results/figures/stock_cov_prob_1stp.pdf"))
print(ggplot(sim_CIs) + geom_point(aes(x = method, y = CI_prob, color = stock_name)) +
  labs(x = "Forecast method", y = "Coverage probability", color = "Stock", title = "1 step forecasts"))
dev.off()


pdf(here("results/figures/1step_coverage_prob.pdf"))
...
colnames(sim_CIs)[1] <- "stock_name"
sim_CIs$stock_name[which(sim_CIs == "blackCA")] <- "black_ca"
sim_CIs$stock_name[which(sim_CIs == "blackWA")] <- "black_wa"
sim_CIs$stock_name[which(sim_CIs == "cabezon_ocs")] <- "cabezon_ors"
sim_CIs$stock_name[which(sim_CIs == "canary1")] <- "canary"
sim_CIs <- left_join(sim_CIs, west_coast_stock_characteristics)
...
b <- ggplot(sim_CIs) + geom_point(aes(x = autocorrR, y = CI_prob, color = method), 
                                  size = 2, position = position_dodge(width = 0.1)) +
  labs(x = "Recruitment autocorrelation", y = "Coverage probability", subtitle = "(b)")
c <- ggplot(sim_CIs) + geom_point(aes(x = num_yrs, y = CI_prob, color = method), 
                                  size = 2, position = position_dodge(width = 0.1)) +
