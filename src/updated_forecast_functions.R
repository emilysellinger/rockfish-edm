...
pred.rec <- function(future_states, mu1, mu2, sd1, sd2, df2, x){
  preds <- rep(NA, (length(future_states)-1))
  
  for(i in 1:(length(future_states)-1)){
    if(future_states[i] == 1){
      logRS <- rnorm(1,mu1,sd1)
      preds[i] <- exp(logRS + log(df2[x]))
    }else{
...
rec_HMM_sample <- function(x, df1, df2, nsims){
  ...
  for(i in 1:nsims){
    rec_preds[i] <- pred.rec(future_states, mu1 = mean(est_state_1$logRS), mu2 = mean(est_state_2$logRS),
